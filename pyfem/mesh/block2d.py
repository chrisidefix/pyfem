...
class Block2D(Block):
    ...
    """
    def __init__(self, coords=None, divisions=(1,1), npoints=4):
        Block.__init__(self)
        self.face_tags = ['', '', '', '']
        self.is_truss  = False
        self.unstruct  = False
    ...
    def set_out_shape(self, shape_type):
        self.out_shape = shape_type

    def set_npoints(self, npoints):
        shape_type = {3:TRI3, 4:QUAD4, 6:TRI6, 8:QUAD8, 9:TRI9, 10:TRI10, 12:QUAD12, 16:QUAD16}.get(npoints, None)
        if shape_type:
            self.set_out_shape(shape_type)
        else:
            raise Exception("Wrong number of points (%d)" % d)
    ...
    def split(self, points, cells, faces):
        ...
        nshp      = self.coords.shape[0]    # number of points of block
        self.sfun = { 4:shape_quad4, 8:shape_quad8, 12:shape_quad12, 16:shape_quad16 }[nshp]
        ...
        if self.out_shape == TRI3:
            self.split_tri3(points, cells, faces)
        if self.out_shape == TRI6:
            self.split_tri6(points, cells, faces)
        if self.out_shape == TRI9:
            self.split_tri9(points, cells, faces)
        if self.out_shape == QUAD4:
            self.split_quad4(points, cells, faces)
        if self.out_shape in [QUAD8, QUAD9]:
            self.split_quad8_9(points, cells, faces)
        if self.out_shape in [QUAD12, QUAD16]:
            self.split_quad12_16(points, cells, faces)

    def split_quad4(self, points, cells, faces):
        nx    = self.nx
        ny    = self.ny
    ...
    def split_quad8_9(self, points, cells, faces):
        nx    = self.nx
        ny    = self.ny
        ...
        for j in range(2*ny+1):
            for i in range(2*nx+1):
                if self.out_shape == QUAD8 and i%2 and j%2: continue # False point
        ...
        for j in range(2, 2*ny+1, 2):
            for i in range(2, 2*nx+1, 2):
                ...
                if self.out_shape == QUAD8:
                    cell = cells.add_new(QUAD8, [p0, p1, p2, p3, p4, p5, p6, p7], self.tag)
                else:
                    p8 = p_arr[i-1][j-1]
                    cell = cells.add_new(QUAD9, [p0, p1, p2, p3, p4, p5, p6, p7, p8], self.tag)
    ...
    def split_quad12_16(self, points, cells, faces):
        nx    = self.nx
        ny    = self.ny
        ...
        for j in range(3*ny+1):
            for i in range(3*nx+1):
                if self.out_shape == QUAD12 and i%3 and j%3: continue # False point
    ...
    def split_tri3(self, points, cells, faces):
        # 
        #  3                        2
        #    @--------------------@
        #    |                  / |
    ...
    def split_tri6(self, points, cells, faces):
        # 
        #  3           6            2
        #    @---------@----------@
        #    |                  / |
    ...
    def split_tri9(self, points, cells, faces):
        #  
        #   3      10       6        2
        #     @-----@-------@------@
        #     |                  / |
